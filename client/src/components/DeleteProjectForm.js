...
export function DeleteProjectForm({toShow, setToShow, projectName, setProjects, projectId, teamId, socket}){

    function handleDelete(){
        axios.delete(`/api/projects/delete/${projectId}`).then(response=>{
            if (response.status === 200){
                setProjects(projects => projects.filter((proj)=>proj._id !== projectId));
                socket.emit('deleteProject', {projectId, teamId})
                setToShow(false);
            }
        }).catch(error=>{
            console.log(error);
